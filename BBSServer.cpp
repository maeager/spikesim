...
boolean BBS_Server::look_take(const char* key, bbsmpibuf** recv)
{
#if debug
...
boolean BBS_Server::look(const char* key, bbsmpibuf** recv)
{
#if debug
...
void BBS_Server::put_pending(const char* key, int cid)
{
#if defined(HAVE_STL)
#if debug
...
boolean BBS_Server::take_pending(const char* key, int* cid)
{
    boolean b = false;
#if defined(HAVE_STL)
...
void BBS_Server::post(const char* key, bbsmpibuf* send)
{
#if defined(HAVE_STL)
    int cid;
#if debug
...
void BBS_Server::add_looking_todo(int cid)
{
#if defined(HAVE_STL)
...
void BBS_Server::post_todo(int pid, int cid, bbsmpibuf* send)
{
#if defined(HAVE_STL)
#if debug
    printf("BBS_Server::post_todo pid=%d cid=%d send=%lx\n", pid, cid, (long)send);
#endif
...
void  BBS_Server::context(bbsmpibuf* send)
{
#if defined(HAVE_STL)
    int cid, j;
#if debug
...
void nrnbbs_context_wait()
{
    if (BBSImpl::is_master_) {
        BBS_Server::server_->context_wait();
    }
}
...
void BBS_Server::post_result(int id, bbsmpibuf* send)
{
#if defined(HAVE_STL)
#if debug
...
int BBS_Server::look_take_todo(bbsmpibuf** recv)
{
#if defined(HAVE_STL)
#if debug
...
int BBS_Server::look_take_result(int pid, bbsmpibuf** recv)
{
#if debug
