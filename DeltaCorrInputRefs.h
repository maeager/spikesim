...
//  CorrInputRefShiftedCopy(DeltaCorrInputRef * reference, unsigned delay_in_timestep);
    ...
    inline const double & rate_when_not_correlated() const {
        return reference_->rate_when_not_correlated();
    }
    inline const double & rate_when_correlated() const {
        return reference_->rate_when_correlated();
    }
    inline const unsigned & delay_in_timestep() const {
        return delay_in_timestep_;
    }
    const bool & correlated_event_state() const;
private:
    void input_update();
