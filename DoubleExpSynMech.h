...
//	DoubleExpSynMechConfig(double tauA, double tauB);
	...
	inline const double & renorm() const {return renorm_;}
private:
...
public:
	...
	~DoubleExpSynMech() {delete[] alphajump_;}
protected:
//	template <class TypeImpl> void send_updated_states(TypeImpl & neuron);
	void send_updated_states(double & current);
	void send_updated_states(double & conductance, double & current) {throw Error("DoubleExpSynMech: should not be used");}
