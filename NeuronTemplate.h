...
public:
	...
	const Volt & potential() const {return NrnActMech::potential_impl();}
	inline void apply_visitor(AbstractVisitor & vis) {NrnData::apply_vis_impl(vis);}
protected:
	inline void update() {NrnActMech::perform_calculations(*this);}
	void add_presynapse(SynapseInterface * const syn) {NrnData::add_presynapse_impl(syn);}
	void add_postsynapse(SynapseInterface * const syn) {NrnData::add_postsynapse_impl(syn);}
};
