...
private:
    ...
    static std::string make_name(const std::string & name_base) {
        return name_base + StringFormatter::IntToStr(outputter_count_++);
    }

// accessors to members
public:
    static inline unsigned & i_outputting_period() {
        return i_outputting_period_;
    }
    static inline const Time & outputting_period() {
        return outputting_period_;
    }
