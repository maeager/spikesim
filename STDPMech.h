...
//	STDPMechConfig(double w_in, double w_out, double tauP, double tauD, double cP, double cD, double cutoffP, double cutoffD, double gmin, double gmax);
	...
	inline const double & cP() const {return STDPFunction::cP_;}
	inline const double & cD() const {return STDPFunction::cD_;}
	inline const double & tauP() const {return STDPFunction::tauP_;}
	inline const double & tauD() const {return STDPFunction::tauD_;}
	inline const double & cutoffP() const {return STDPFunction::cutoffP_;}
	inline const double & cutoffD() const {return STDPFunction::cutoffD_;}
	inline const double & w_in() const {return w_in_;}
	inline const double & w_out() const {return w_out_;}
	inline const double & gmin() const {return STDPBounds::min_;}
	inline const double & gmax() const {return STDPBounds::max_;}
	inline void weight_change(double & weight, const double & dt) const {weight += STDPFunction::weight_change_impl(weight, dt);}
	inline void check_bounds(double & weight) const {STDPBounds::check_bounds_impl(weight);}
private:
	double w_in_, w_out_;
};
